...
- `runtime/platforms.json` — example platform registry (hosts, tiers)
- `runtime/shared_instructions.md` — shared runtime behavior
...
The gate lives in `mcp-server/src/routing_selection_runtime.ts`
...
- Canonical agent contracts live in `runtime/agents/`. The OpenCode
  copies under `opencode-orchestration/opencode/agents/` are specialized, not
  generated — `npm run drift:check` is the gate.
- Prettier is not applied to `packs/` (vendored upstream content)

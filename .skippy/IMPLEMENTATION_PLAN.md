...
- [x] Create `xx-stack/mcp-server/src/verify_edit_tools.test.ts` with tests for pass/fail/truncation/policy path
- [x] Register `verify_edit` tool in `xx-stack/mcp-server/src/index.ts` via `registerVerifyEditTools(server, deps)`
- [x] Run `npm run verify` to confirm all checks pass

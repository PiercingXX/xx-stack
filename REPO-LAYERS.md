...
Shims are safe to remove when **all** of the following hold:

1. No script, generator, or eval fixture in this repo references the legacy root path.
2. No documented downstream consumer (e.g. a downstream repo that ran `./setup-vscode.sh`) pins the legacy path.
3. `node scripts/verify-repo-layout.mjs` still passes after the shim is deleted.

Remove each shim individually rather than in bulk so regressions are easy to bisect.
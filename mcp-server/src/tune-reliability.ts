 ...
 * Paste the suggested patch into opencode/config.json (or the installed compatibility shim) and run `npm run harness:ci`
...
async function main(): Promise<void> {
  ...
  if (logFiles.length === 0) {
    process.stdout.write("No harness logs found. Run `npm run harness:ci` at least once first.\n");
    process.exit(0);
  }
  ...
  for (const [name, durations] of byScenario.entries()) {
    ...
    process.stdout.write(
      `    p50=${p50}ms  p90=${p90}ms  p99=${p99}ms  samples=${sorted.length}\n\n`
    );
  }

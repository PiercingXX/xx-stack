...
What it does:
  ...
  2. In workspace mode, creates/updates <target>/.vscode/mcp.json
...
Options:
  --global  Install agents and prompts for all VS Code workspaces.
...
if [[ $GLOBAL -eq 1 ]]; then
  ...
  echo "[xx-stack] Installing agents to user-level VS Code config..."
  cp -f "$STACK_DIR"/vscode/agents/*.agent.md "$USER_PROMPTS_DIR"/
  echo "[xx-stack] Installing prompts (skills) to user-level VS Code config..."
  cp -f "$STACK_DIR"/vscode/skills/*.prompt.md "$USER_PROMPTS_DIR"/
  echo "[xx-stack] Installation complete."
  echo "[xx-stack] Installed to: $USER_PROMPTS_DIR"
  echo "[xx-stack] Reload VS Code to see agents in Copilot Chat."
  exit 0
fi

TARGET_DIR="$(cd "$TARGET_PATH" && pwd -P)"
VSCODE_DIR="$TARGET_DIR/.vscode"
GITHUB_AGENTS_DIR="$TARGET_DIR/.github/agents"
GITHUB_PROMPTS_DIR="$TARGET_DIR/.github/prompts"
...
if [[ -f "$GLOBAL_AGENT_SENTINEL" ]]; then
  ...
  cp -f "$STACK_DIR"/vscode/agents/*.agent.md "$GITHUB_AGENTS_DIR"/
  cp -f "$STACK_DIR"/vscode/skills/*.prompt.md "$GITHUB_PROMPTS_DIR"/
fi
...
echo "[xx-stack] Linked VS Code workspace: $TARGET_DIR"
echo "[xx-stack] Wrote: $MCP_CONFIG_PATH"
if [[ -f "$GLOBAL_AGENT_SENTINEL" ]]; then
  echo "[xx-stack] Using global agents/prompts from: $USER_PROMPTS_DIR"
else
  echo "[xx-stack] Synced: $GITHUB_AGENTS_DIR"
  echo "[xx-stack] Synced: $GITHUB_PROMPTS_DIR"
fi
echo "[xx-stack] Design pack symlinked at project root."
echo "[xx-stack] Ready. Reload VS Code window in the target workspace."

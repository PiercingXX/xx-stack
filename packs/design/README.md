...
repository. Do not reformat it — `packs/` is in `.prettierignore` so
...
all-pairs cap are the method used by the `dataviz` skill bundled with Claude
...
- `node packs/design/scripts/check-craft-references.mjs` — every
...
- `node packs/design/scripts/test-anti-slop-rules.mjs` — drives the

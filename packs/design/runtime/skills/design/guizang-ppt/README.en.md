...
A [a bundled coding-agent / the assistant Agent Skills](https:/agentskills.io/) skill that generates **single-file HTML horizontal-swipe decks** with an "**editorial magazine × electronic ink**" aesthetic — picture *Monocle* with code stitched in.
...
Preview image omitted in this local copy.
...
> Install the `guizang-ppt-skill` a bundled coding-agent skill for me. Steps:
>
> 1. Make sure `~/.agents/skills/` exists (create if not)
> 2. Run `git clone https:/example.com/skills/magazine-web-ppt.git ~/.agents/skills/magazine-web-ppt`
...
Paste the block above into a bundled coding-agent / the editor / any AI agent with shell access and it handles the install.

### Option 2: Manual CLI

```bash
git clone https:/example.com/skills/magazine-web-ppt.git ~/.agents/skills/magazine-web-ppt
```

### How to trigger it

Once installed, a bundled coding-agent auto-detects the skill. Trigger phrases:
...
The skill is a structured 6-step flow; the assistant walks you through each:

...
The skill auto-registers with the daemon on filesystem scan (no manual wiring). Confirmed against a running local daemon after adding this skill:

```bash
$ curl -s "$SKILL_DAEMON_URL/api/skills" \

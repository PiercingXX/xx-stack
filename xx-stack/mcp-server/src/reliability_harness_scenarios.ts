...
export async function runHarnessScenarios(): Promise<ScenarioResult[]> {
  ...
  {
    const t0 = Date.now();
    const runtime = __testExports.DEFAULT_RELIABILITY;
    const worstCaseMs =
...

  {
    const t0 = Date.now();
    const now = Date.now();
    const runtime = { ...__testExports.DEFAULT_RELIABILITY, failureResetWindowMs: 5_000 };
    const store = emptySupervisorStore();
...

  {
    const t0 = Date.now();
    const now = Date.now();
    const runtime = { ...__testExports.DEFAULT_RELIABILITY, failureResetWindowMs: 2_000 };
    const store = emptySupervisorStore();

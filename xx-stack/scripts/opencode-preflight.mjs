...
const phaseTimeoutMs = parsePositiveInt(
  args["phase-timeout-ms"],
...
try {
  ...
  if (pingResult.code !== 0 || pingResult.timedOut || !pingResult.stdout.includes("PONG")) {
    process.stderr.write(
      `PING_FAILED code=${pingResult.code} signal=${pingResult.signal ?? "none"} timedOut=${pingResult.timedOut ? "yes" : "no"}\n`
    );
  ...
  if (toolResult.code !== 0 || toolResult.timedOut || !toolOutput.includes(probeMarker)) {
    process.stderr.write(
      `TOOL_LOOP_UNSUPPORTED agent=${workerAgent} code=${toolResult.code} signal=${toolResult.signal ?? "none"} timedOut=${toolResult.timedOut ? "yes" : "no"}\n`
    );

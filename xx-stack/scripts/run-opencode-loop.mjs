...
async function buildIsolatedRuntime({ stackRoot, isolatedHome, sourceConfigPath }) {
  ...
  const nextConfig = {
    $schema:
      typeof hostConfig.$schema === "string"
        ? hostConfig.$schema
        : typeof runtimeConfig.$schema === "string"
...
const sourceConfigPath = resolve(
  args["host-config"] ?? join(process.env.HOME ?? "", ".config", "opencode", "config.json")
);
